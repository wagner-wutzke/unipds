...
//        categoriasEmPromocao.put(ENTRADAS, " Comece sua refeicao com toque de sabor!");
    ...
    public boolean removerItemCardapio(Long itemId) {
        ItemCardapio itemCardapio = itensPorId.remove(itemId);
        return itemCardapio != null;
    }

    public boolean alterarPreco(Long itemId, BigDecimal novoPreco) {
        ItemCardapio itemAnterior = itensPorId.get(itemId);
        if (itemAnterior == null) return false;
        ItemCardapio alterado = itemAnterior.alterarPreco(novoPreco);
        itensPorId.put(itemId, alterado);
        auditoriaPrecos.put(itemAnterior, novoPreco);
        return true;
    }

    public void rastroAuditoriaPrecos() {
        System.out.println("\n=========================================");
        System.out.println("AUDITORIA DE PRECOS");
        System.out.println("=========================================");
        auditoriaPrecos.forEach((itemCardapio, novoPreco) -> {
            System.out.printf("- %s: %s => %s\n", itemCardapio.nome(), itemCardapio.preco(), novoPreco);
        });
